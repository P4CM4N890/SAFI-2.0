...
const checkFiles = async (uuid: number) => {
...
export const createNotification = async (tituloMeta: string, fecha_final: Date, uuid: number) => {
    try {
        const path = RNFS.DocumentDirectoryPath + `/notificaciones${uuid}.json`;
        const pathId = RNFS.DocumentDirectoryPath + `/lastId${uuid}.json`;
        let notificaciones = await checkFiles(uuid);
